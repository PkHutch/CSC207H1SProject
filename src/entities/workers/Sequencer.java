	...
	public void doTask() {
		...
		for (int i = 0; i < pickingRequests.size(); i++) {
			...
			while (pickingRequests.get(i).getStatus() == 3) {
				...
				l++;
				Stock[] front = { new Stock(sequenced[0]), new Stock(sequenced[2]), new Stock(sequenced[4]),
						new Stock(sequenced[6]) };
				Pallet pallet = new Pallet(front);

				this.getWarehouse().getMarshalling().addPallet(pallet);

				Stock[] back = { new Stock(sequenced[1]), new Stock(sequenced[3]), new Stock(sequenced[5]),
						new Stock(sequenced[7]) };
