...
public class Picker extends Worker implements TaskExecutor<String> {
     ...
     *            to 8. This command should only be after setReady() has been
     *            called, and the server has been assigned a picking request by
     *            the server.
     */
            ...
            if (this.isActive == false) {
                try {
                    ...
                    if (currentPick == pickInteger - 1 && currentPick < DEFAULT_PICK_END) {
                        ...
                        throw new IllegalStateException("The Picker \"" + this.getName()
                                + "\" is finished the current picking request, they should be sent to marshalling.");
                    } else {
                        throw new IllegalArgumentException(
                                "The Picker \"" + this.getName() + "\" is not on the given picking number they are on "
                                        + Integer.toString((this.currentPick + 1)) + ".");
                    }
                } catch (NumberFormatException exception) {
                    throw new IllegalArgumentException("The Picker \"" + this.getName()
                            + "\" can not pick invalid number \"" + pickNumber + "\".");
                }
            } else {
                throw new IllegalStateException(
                        "The Picker \"" + this.getName() + "\" is not currently checked in as ready.");
            }
        } else {
            throw new IllegalStateException(
                    "The Picker \"" + this.getName() + "\" does not currently have a picking request assigned.");
        }
    }
    ...
    private void setPickingLocations(List<String> newPickingLocations) {
        for (int index = 0; index < newPickingLocations.size(); index++) {
            ...
            this.pickingLocations.add(
                    this.getWarehouse().getFloor().getLevel(newLocation[0].charAt(0), Integer.parseInt(newLocation[1]),
    ...
    public void setPickingRequest(PickingRequest newPickingRequest) {
        if (this.hasPickingRequest() == false) {
            ...
            this.setPickingLocations(new LinkedList<String>(this.getWarehouse().getWarehousePicking()
                    .optimize(new ArrayList<Integer>(Arrays.asList(newPickingRequest.getSKUs())))));
    ...
    public void setReady() {
        if (this.hasPickingRequest() == false) {
            if (this.isActive == true) {
                ...
                throw new IllegalStateException("The picker \"" + this.getName()
                        + "\" is waiting for a picking request, more orders must be sent in.");
            }
        } else {
            throw new IllegalStateException("The picker \"" + this.getName()
                    + "\" is already picking a picking request, the pick command should be used.");
        }
    }
